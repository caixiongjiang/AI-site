...
function KnowledgeWorkspace() {
  ...
  const handleFileSelect = async (
    ...
    try {
      ...
      setNotice(`已上传 ${uploaded.length} 个文件，系统正在处理，很快就可以提问。`);
    } catch (error) {
      setNotice(
  ...
  const handleOpenFile = (file: KnowledgeFile) => {
    ...
    window.open(`/knowledge/file/${encodeURIComponent(file.file_id)}`, "_blank", "noopener,noreferrer");
  };
  ...
  return (
    <div className="flex h-screen bg-[radial-gradient(circle_at_top,rgba(0,179,107,0.12),transparent_28%),linear-gradient(180deg,#121516_0%,#0f1112_100%)]">
      ...
      <main className="grid flex-1 grid-cols-1 overflow-hidden xl:grid-cols-[minmax(540px,0.95fr)_minmax(620px,1.2fr)]">
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-3 xl:grid-cols-4">
                  ...
                  <div className="rounded-[24px] border border-white/5 bg-[linear-gradient(160deg,rgba(0,179,107,0.18),rgba(37,37,38,0.95))] p-5">
                    ...
                    <div className="mt-3 text-sm leading-6 text-foreground">
                      问答区会默认展开。点击任意文件后，会在新页面中打开 PDF 与单文件问答。
                    </div>
                  </div>
                </div>
        ...
        <div className="overflow-y-auto p-4 md:p-5">
          <KnowledgeChatPanel
            ...
            subtitle={
              selectedKb
                ? `当前围绕「${selectedKb.knowledge_base_name}」展开问答，问答区默认打开并占据页面主体。`
